...
Data *SearchDArray(DArray *DArrayHead, char *String) {
   ...
   //iterate over every element until a match is found
   for ( count = 0; count  <= DArrayHead -> EntriesUsed; count ++ ) {
      if ( !strcmp( String, DArrayHead -> Payload[count].String )) {
         return &DArrayHead -> Payload[count]; 
         //word found = return word from DArray
      }
   }
   //if entire array is searched return NULL (word not found)
   return NULL;
} /* SearchDArray() */
