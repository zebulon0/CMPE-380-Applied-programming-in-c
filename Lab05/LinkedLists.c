...
/*-----  Don't implement this until instructed in a future lab ----*/
   LinkedListNodes *tempNode;

   //error checking
   if ( ListPtr == NULL || ListPtr -> NumElements == 0) { return 0; }
   
   else {//if no issues with the pointer or Linked List, start searching..
      tempNode = ListPtr -> FrontPtr;
      //set the tempNode to the first node in the LL
      
      while ( tempNode != NULL ) {//while the current node is not the last node (NULL)...
         
         //if the given word to search for matches the current node's String
         //exit loop and return 1 because the word was found
         if ( !strcmp( String, tempNode -> String )) { return 1; }
        
         //get the next node and check its string
         tempNode = tempNode -> Next;
      }
      //no words found? take the L and return a 0 :(
      return 0;
   }
} /* SearchList() */
