...
double secant(func1arg f, double x0, double x1, int Nmax, double atol, int verb) {
    ...
    double fx0;		/* Value of f(x0) */
    double fx1;		/* Value of f(x1) */
    int i=0;		/* Iteration */
    double temp;	/* Holds the xn-1 value */
    
    if(fabs(f(x0))< atol){ return x0; }
	...
	/* Check for case where denominator is 0 or very close to 0*/
        //if ( fabs(fx1-fx0) < atol ) { 
